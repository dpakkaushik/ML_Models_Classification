...
def main():
    ...
    competition_open_since_year = st.number_input("competition_open_since_year",min_value=1900,max_value=2015,step=1e-6,format="%.5f")
    competition_distance = st.number_input("competition_distance",min_value=-1,max_value=32,step=1e-6,format="%.5f")
    store = st.number_input("store",min_value=1,max_value=1115,step=1e-6,format="%.5f")
    competition_open_since_month = st.number_input("competition_open_since_month",min_value=1,max_value=12,step=1e-6,format="%.5f")
    store_type_d = st.number_input("store_type_d",min_value=0.,max_value=1.0,step=1e-6,format="%.5f")
    day_cos = st.number_input("day_cos",min_value=-1,max_value=1.0,step=1e-6,format="%.5f")
    ...
    promo2_since_year = st.number_input("promo2_since_year", min_value=2009, max_value=2015, step=1e-6, format="%.5f")
    competition_time_month = st.number_input("competition_time_month", min_value=-1, max_value=19, step=1e-6, format="%.5f")
    promo2_since_week = st.number_input("promo2_since_week", min_value=0., max_value=50, step=1e-6, format="%.5f")
    assortment = st.number_input("assortment", min_value=0., max_value=3, step=1e-6, format="%.5f")
